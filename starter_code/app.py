...
class Venue(db.Model):
    ...
    def to_dict(self):
        # returns all venues as dictionary
        return {
            'id': self.id,
            'name': self.name,
            'city': self.city,
            ...
            'genres': self.genres,  # convert string to list
...
class Artist(db.Model):
    ...
    def to_dict(self):
      # return all aritists as dictonary
        return {
            'id': self.id,
            'name': self.name,
            'city': self.city,
...
def search_venues():
  ...
  for venue in venues:
        tmpt = {}
        tmpt['id'] = venue.id
        tmpt['name'] = venue.name
        tmpt['num_upcoming_shows'] = len(venue.shows)
        data.append(tmpt)
...
# def show_venue(venue_id):
  ...
  # pts = list(filter(lambda x: x.start_time < datetime.today(), venue.shows))
  # ups = list(filter(lambda x: x.start_time >= datetime.today(), venue.shows))

  # past_shows = list(map(lambda x: x.show_artist(), pts))
  # upcoming_shows = list(map(lambda x: x.show_artist(), ups))
...
def search_artists():
  ...
  # return render_template('pages/search_artists.html', results=response, search_term=request.form.get('search_term', ''))
    ...
    results = Artist.query.filter(Artist.name.ilike('%{}%'.format(search_item))).all()  
...
def show_artist(artist_id):
  ...
  for show in upcoming_shows:
      future_shows = {}
      future_shows["venue_name"] = show.venues.name
      future_shows["venue_id"] = show.venue.id
      future_shows["venue_image_link"] = show.venues.image_link
      future_shows["start_time"] = show.start_time.strftime("%m/%d/%Y, %H:%M:%S")
      all_future_shows.append(future_shows)
  ...
  for show in past_shows:
        pst_shs = {}
        pst_shs["venue_id"] = show.venues.id
        pst_shs["venue_name"] = show.venues.name
        pst_shs["venue_image_link"] = show.venues.image_link
        pst_shs["start_time"] = show.start_time.strftime("%m/%d/%Y, %H:%M:%S")
        all_past_shows.append(pst_shs)
